...
export
const removeApostrophe = (str: string): string => str.replace(/.'/, '');

const normaliseSpelling = (builder: Object) => {

  const pipelineRemoveApostrophe = (token: Object) => {
      return token.update( str => removeApostrophe(str) );
  }

  const pipelineRemoveDiacritics = (token: Object) => {
      return token.update( str => removeDiacritics(str) );
  }
  ...
  lunr.Pipeline.registerFunction(pipelineRemoveDiacritics, 'removeDiacritics')
	...
	builder.pipeline.before(lunr.stemmer, pipelineRemoveDiacritics)
  builder.searchPipeline.before(lunr.stemmer, pipelineRemoveDiacritics)  

  builder.pipeline.before(lunr.stemmer, pipelineRemoveApostrophe)
  builder.searchPipeline.before(lunr.stemmer, pipelineRemoveApostrophe)
};
